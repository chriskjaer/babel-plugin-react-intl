    ...
    visitor: {
      JSXOpeningElement(path, state) {
        ...
        if (referencesImport(name, moduleSourceName, COMPONENT_NAMES)) {
          ...
          if (descriptor.defaultMessage) {
            ...
            let hasNewId = false
            if (state.opts.prependFolderNameToId) {
              if (descriptor.id[0] === '.') {
                const folderName = p.basename(
                  p.dirname(state.file.opts.filename)
                )
                descriptor.id = folderName + descriptor.id
                hasNewId = true
              }
            }
            ...
            let descriptionRemoved
            let idReplaced = !hasNewId
            for (let attr of attributes) {
              ...
              } else if (key === 'id' && hasNewId) {
                // Replace the id in the code if we have generated a new one
                attr.node.value.value = descriptor.id
                idReplaced = true
              }

              if (descriptionRemoved && idReplaced) break
            }
      ...
      CallExpression(path, state) {
        ...
        function processMessageObject(messageObj) {
          ...
          if (state.opts.prependFolderNameToId) {
            if (descriptor.id[0] === '.') {
              const folderName = p.basename(p.dirname(state.file.opts.filename))
              descriptor.id = folderName + descriptor.id
            }
          }
